...
| 27 | Source listing UX | Rely on `wiki/index.md` + Obsidian / `/llm-wiki:query "list sources"` / **Dedicated `/llm-wiki:sources` command** | **Dedicated read-only command, compact Markdown table** (v0.4.4) | First real-use question was "how do I see all my sources without Obsidian"; index.md works but isn't structured, query reads as natural-language not a list. Symmetric with `/llm-wiki:list` (vaults) → `/llm-wiki:sources` (sources within a vault). Read-only, no log entry — pure inspection |
| 28 | Research filter-out display | Show all candidates including 1-star with rejection annotation / **Hide 1-star, summarize count at bottom** / Two separate sections | **Hide 1-star with a one-line "Filtered out: K …" summary** (v0.4.5) | First real research run produced confusing output where 1-star (off-topic) candidates appeared as unnumbered blank rows between numbered ones — the LLM was improvising because Step 3 didn't specify how to handle filtered items. Hiding them entirely is cleaner; users would never select them anyway. Filter rationale stays available implicitly via the count (could surface details with a future `--verbose` flag) |
...
| v0.4.4 | 2026-05-06 | New `/llm-wiki:sources` command — read-only listing of all sources in the active vault (slug, title, year, authors, branch). Symmetric with `/llm-wiki:list` for vaults |
| v0.4.5 | 2026-05-07 | `/llm-wiki:research` filters 1-star (off-topic) candidates from the presented list — they appear only as a one-line "Filtered out: K candidates" summary at the bottom, eliminating the confusing unnumbered-row improvisation |
